...
def export(run):
    ...
    for id in ids:
        ...
        modules=[
          ...
          dict(id='M1',name='检索与材料整理',action='助手取得官方材料，保存结构并核对坐标、顺序与单位；材料取得方式及来源记录时间见清单。',inputs=[record('sources.json',{k:sources[k] for k in ('search_queries','selection_unit','resources')})],outputs=[record('bundle.json',{'paper_id':bundle['paper_id'],'title':bundle['title'],'sources':manifest,'omitted':'Source text omitted; full XYZ available in M6 output and question detail.'}),record('asset-units.json',units)]),
          dict(id='M2',name='科学证据提取',action='按保存的提示词提取条件、坐标依据与限制。证据可能由同一批次题目共享。',inputs=[packet('evidence')],outputs=[record('evidence.json',ev)]),
          ...
          dict(id='M4',name='构题与答案计算',action=('构题计划指定评分器，由通用程序计算数值答案。' if item['verifier'] else '本题使用 evidence_review，参考答案包含解释；通用流程不自动核验解释。若有补充数值计算，其复核记录在 M5；QNP001 使用 force_projection.py 和 Decimal。'),inputs=[packet('construction')],outputs=[record('construction-receipt.json',{'plan':{'items':[x for x in receipt['plan']['items'] if x['qa_id']==id]}}),record('qa.json',{'stage':qa['stage'],'time':qa['time'],'data':{'items':[item]}})]),
