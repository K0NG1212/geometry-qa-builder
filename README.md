...
你可以在当前任务中说：**“按 RUNBOOK.md，使用 GPT‑6 对这些材料运行一轮，保存每个阶段和验证报告。”** 无需把 API 密钥发到聊天。当前任务的模型以应用设置为准；脚本不能替应用切换模型。应用内执行与 API 实验分别记录，不能混称同一次可复现实验。

批量 API 模式默认 `gpt-6-astra`，只读取本机环境变量 `OPENAI_API_KEY`；不会自动降级到别的模型。API 与应用内运行是两个入口。
