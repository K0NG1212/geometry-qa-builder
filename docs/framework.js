...
(() => {
  ...
  const render = () => {
    ...
    for(const [domain,label] of Object.entries(domains)){
      ...
      for(let i=0;i<4;i++){
        ...
        const all=catalog.questions.filter(q=>q.domain===domain&&inBin(q[field],i));
        const candidates=all.filter(q=>!ability||q.ability===ability);
        const passed=all.filter(q=>q.prototypeScreeningPassed===true&&q.status==='pending_human_audit');
    ...
    const unlocated=catalog.questions.filter(q=>!bins.some((_,i)=>inBin(q[field],i)));
  ...
  Promise.all(['data/task-framework.json','data/task-reviews.json','data/task-readiness-v04.json','data/catalog.json'].map(url=>fetch(url).then(r=>{if(!r.ok)throw Error(r.status);return r.json()}))).then(([f,reviews,newReview,questions])=>{
    ...
    section.querySelector('.caption').textContent='规划方向使用拟议尺度，候选题使用实测尺度，二者不相互冒充。默认按推理尺度规划每格10道、各能力尽量3道。切换输入视角只改变分布，不重新定义配额。单元格进度为全能力统计，不随筛选变化；能力筛选同时影响方向与候选，处理建议只筛选规划方向。';
    ...
    const screened=catalog.questions.filter(q=>q.prototypeScreeningPassed===true&&q.status==='pending_human_audit').length;
    const latest=document.createElement('p');latest.className='review-summary';
    latest.innerHTML=`新版规则就绪审查 · ${esc(newReview.date)}<br>已有新版试跑方向 1 · 缺材料／评分器 16 · 先修订题型 3 · 设计暂不可执行 7 · 合并方向 4。<br>原型初筛 ${screened} / 160 · 正式可用 ${catalog.counts.ready}。这是方向级审查，未将31项都当作完整QA运行。<br><a class="text-link" href="data/task-readiness-v04.json" download>下载新版审查记录 ↓</a> · <a class="text-link" href="chem-local-batch.html">最新实际试跑报告 ↗</a>`;
    ...
    $('#coverage').addEventListener('click',e=>{
      ...
      $('#detail-content').innerHTML=`<div class="eyebrow">${t[0]} / SOURCE REVIEW · ${esc(reviews.date)}</div><h2 id="detail-title">${esc(t[6])}</h2><div class="detail-meta">${badge(t[3])}<span class="badge">${esc(domains[t[1]])} › ${esc(t[2])}</span><span class="badge">${esc(r.decision_label)}</span></div><p class="caption">原提案分类与规划位置：输入 ${bins[t[4]]}；推理 ${bins[t[5]]}。实例长度未核实。</p><div class="detail-section"><h3>新版 Builder v0.4 就绪审查</h3><p><b>${esc(readiness.reviews[t[0]].label)}</b><br>${esc(readiness.reviews[t[0]].reason)}</p><p>这是方向级材料与方法检查，不是该方向所有 QA 已通过。</p>${readiness.reviews[t[0]].linked_candidates.map(id=>`<button class="text-button" data-question="${id}">${id} · 查看实际题目</button>`).join(" ")}</div><div class="detail-section"><h3>处理理由 · ${esc(r.support_level)}</h3><p>${esc(r.reason)}</p><h3>下一动作</h3><p>${esc(r.next_action)}</p></div><div class="detail-section"><h3>原提案问法</h3><p>${esc(t[7])}</p><h3>查证后的构题建议</h3><p>${esc(r.construction)}</p><h3>验证方法</h3><p>${esc(r.verification)}</p><h3>所需输入</h3><p>${esc(t[8])}。${esc(r.input_assets)}。</p></div><div class="detail-section"><h3>具体来源与支持边界</h3>${r.source_ids.map(id=>{const s=reviews.sources[id];return `<div class="source-evidence"><a class="text-link" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">${esc(s.title)} ↗</a><p>定位：${esc(s.locator)}<br>${esc(s.finding)}</p></div>`}).join('')}<h3>现有候选</h3><p>${t[11]?esc(t[11])+'（状态不因本轮查证自动升级）':'尚无具体候选实例'}</p><p class="caption">${esc(r.benchmark_relation)} 首轮资料查证由助手完成，未经过独立领域审核；处理结论是研究建议。</p></div>`;
