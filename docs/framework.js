...
(() => {
  ...
  Promise.all(['data/task-framework.json','data/task-reviews.json'].map(url=>fetch(url).then(r=>{if(!r.ok)throw Error(r.status);return r.json()}))).then(([f,reviews])=>{
    ...
    section.querySelector('.caption').textContent='等宽区间对应 log₁₀ 长度轴；左闭右开，最右端包含 1000 nm。1 Å = 0.1 nm，1 nm = 10⁻⁹ m，1 μm = 1000 nm。任务按拟议情境放置，具体对象尺寸可跨区间；尚无实测值的 26 条候选不自动获得数值坐标。';
    ...
    scaleArticle.innerHTML='<h2>01 / 两种尺度与数值横轴</h2><p>输入尺度描述给模型的对象范围；推理尺度描述解题需关注的范围。完整蛋白质的输入和原子接触的推理可以落在不同区间。</p><p>本版横轴显示 10⁻¹⁰、10⁻⁹、10⁻⁸、10⁻⁷、10⁻⁶ m 五个边界，即 0.1、1、10、100、1000 nm。等宽区间在对数轴上跨越一个数量级；这是研究规划窗口，不是对象类别的固定尺寸。超出范围的任务需扩展坐标轴。</p><p>有限分子的工作测量定义可采用所选原子集合的最大原子间距；局部任务采用任务所需子集。周期晶体需声明晶胞、超胞或实际关注范围，不能把无限晶体设为一个有限直径。必须先核实单位、坐标与协议，再填实例尺度。当前表中的位置均为拟议范围。</p><p><a class="text-link" href="https://www.bipm.org/en/measurement-units/si-prefixes" target="_blank" rel="noopener noreferrer">SI 前缀依据 ↗</a></p>';
    render();
