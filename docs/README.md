...
保留 framework.js 主地图及尺度、能力、处理建议筛选。单元格中分开显示实测候选、规划方向与全能力初筛统计。prototype.js 不再加载，重复的 measured-coverage 不再生成。输入视角展示分布，不将输入格误当推理配额。26 条尚无实测尺度的旧记录保留待定位入口。task-readiness-v04.json 是新版规则方向级就绪审查，不是31次完整QA运行；原2026-09-14资料与建议保留，未重新宣称已核验论文。
